...
void main() {
  ...
  group("OpenCryptoPaySendHandler.sendFee", () {
    ...
    testWidgets("an EVM preset at or above the minimum is kept", (
      ...
      // base 10 + tip 2 = 12 gwei meets the minimum.
      final customFee = EthEIP1559Fee(
        maxFeePerGasGwei: Decimal.fromInt(15),
        maxPriorityFeePerGasGwei: Decimal.fromInt(2),
        gasLimit: 90000,
      );
      final custom = await feeFor(
        tester,
    ...
    testWidgets("an EVM custom fee is judged by base fee plus tip", (
      tester,
    ) async {
      final harness = await _pumpHarness(tester);
      final setup = await pendingErc20(tester, harness, minFee: 12000000000);
      // The cap allows 15 gwei but base 10 + tip 1 pays only 11 gwei.
      final floor = await feeFor(
        tester,
        harness,
        setup.handler,
      ...
      expect(fee.maxFeePerGasWei, gwei * BigInt.from(22));
      expect(fee.maxPriorityFeePerGasWei, gwei * BigInt.two);
    });
  ...
  group("OpenCryptoPaySendHandler.submitProof", () {
    ...
    testWidgets("retry resubmits the proof until the user cancels", (
      tester,
    ) async {
      final harness = await _pumpHarness(tester);
      var proofRequests = 0;
      final setup = _makeHandler(
        ...
        client: _mockOcpServer(
          ...
          proofStatus: 500,
          onRequest: (url) {
            if (url.path.contains('/tx/')) proofRequests++;
          },
        ),
      );
      await _handle(tester, harness, setup.handler);
    ...
    testWidgets("a rejection after a lost response keeps the unconfirmed "
        "wording", (tester) async {
      final harness = await _pumpHarness(tester);
      var proofRequests = 0;
      final paymentInfo = _paymentInfoJson(
        quoteExpiration: _futureExpiration(),
      );
      final txDetails = _btcDetailsJson(hint: _hexHint);
      final setup = _makeHandler(
        harness: harness,
        coin: Bitcoin(CryptoCurrencyNetwork.main),
        client: MockClient((request) async {
          if (request.url.path.contains('/tx/')) {
            if (proofRequests++ == 0) throw Exception("socket closed");
            return Response('{}', 400);
          }
          if (request.url.queryParameters.containsKey('method')) {
            return Response(jsonEncode(txDetails), 200);
          }
          return Response(jsonEncode(paymentInfo), 200);
        }),
      );
      await _handle(tester, harness, setup.handler);

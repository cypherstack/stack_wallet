...
command -v xcodebuild >/dev/null || {
  echo "Xcode is required for macOS builds." >&2
  exit 69
}
command -v rustup >/dev/null || {
  echo "rustup is required for macOS builds." >&2
  exit 69
}
case "$(uname -m)" in
...
safe_remove_output build/macos
...
"$STAGEX_PROJECT_ROOT/.flutter-sdk/flutter/bin/flutter" \
  build macos --release
artifact="$(find build/macos/Build/Products/Release \

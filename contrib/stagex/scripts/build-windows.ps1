...
$BuildState = Join-Path $ProjectRoot '.stagex-build-state\windows'
...
$env:RUSTUP_HOME = Join-Path `
    (Join-Path $ProjectRoot '.rustup-stagex-windows') `
    $PinnedRustVersion
...
$env:STACK_STAGEX_REAL_RUSTUP = $RustupCommand.Source
& $env:STACK_STAGEX_REAL_RUSTUP set auto-self-update disable
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& $env:STACK_STAGEX_REAL_RUSTUP toolchain install $PinnedRustVersion --profile minimal
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
$RustVersion = & $env:STACK_STAGEX_REAL_RUSTUP run $PinnedRustVersion rustc --version
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
...
# Cargokit searches for rustup.exe directly, so pin its moving fallback.
$CargokitBuilders = @(

        ...
        children: [
          if (kDebugMode)
            DetailItem(
              ...
              button:
                  Util.isDesktop
                      ? tvd.IconCopyButton(data: ref.watch(pFrostTxData)!.raw!)
                      : SimpleCopyButton(data: ref.watch(pFrostTxData)!.raw!),
            ),
          ...
          const SizedBox(height: 12),
          DetailItem(title: "Signers", detail: signers),
          const SizedBox(height: 12),
              ...
              try {
                ...
                final txData = await showLoading(
                  whileFuture: ref
                      ...
                      .confirmSend(txData: ref.read(pFrostTxData)!),
                  context: context,
...
class _Recipient extends StatelessWidget {
  const _Recipient({super.key, required this.address, required this.amount});
  ...
  Widget build(BuildContext context) {
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        DetailItem(title: "Address", detail: address),
        const SizedBox(height: 6),
        DetailItem(title: "Amount", detail: amount, horizontal: true),
      ],
    );
  }
}

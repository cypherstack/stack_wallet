 ...
 * Generated by Cypher Stack on 2026-01-12
 *
 */
...
class EpicFinalizeView extends ConsumerStatefulWidget {
  const EpicFinalizeView({
    super.key,
    required this.walletId,
    this.clipboard = const ClipboardWrapper(),
  });

  static const String routeName = "/epicFinalizeView";

  final String walletId;
  final ClipboardInterface clipboard;

  @override
  ConsumerState<EpicFinalizeView> createState() => _EpicFinalizeViewState();
}

class _EpicFinalizeViewState extends ConsumerState<EpicFinalizeView> {
  ...
  Future<void> _finalize() async {
    ...
    if (!decoded.success) {
      throw Exception(decoded.error ?? "Failed to decode slate");
    }

    final analysis = await wallet.analyzeSlatepack(_slateController.text);
    if (analysis.status != "S2") {
      throw Exception("Invalid slate type: ${analysis.status}");
    }
        ...
        children: [
          ClipRRect(
            ...
            child: TextField(
              minLines: 1,
              maxLines: 5,
              key: const Key("epicFinalizeSlateFieldKey"),
                        ...
                        child: Row(
                          ...
                          children: [
                            _slateToggleFlag
                                ? TextFieldIconButton(
                                    key: const Key(
                                      "epicSlateFinalizeClearFieldButtonKey",
                                    ),
                                    onTap: () {
                                      _slateController.text = "";
                                      setState(() {
                                ...
                                : TextFieldIconButton(
                                    key: const Key(
                                      "epicSlateFinalizePasteFieldButtonKey",
                                    ),
